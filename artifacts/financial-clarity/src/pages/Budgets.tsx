...
export default function Budgets() {
  ...
  const unbudgetedCategories = useMemo(() => expenseCategories.filter(c => !budgets.find(b => b.categoryId === c.id)), [expenseCategories, budgets]);
  ...
  const budgetsWithData = useMemo(() =>
    ...
    [budgets, categories, getSpentForCategory, selectedMonth]
  );

  const handleAddBudget = () => {
    if (!newCatId || !newLimit || parseFloat(newLimit) <= 0) return;
    addBudget({ categoryId: newCatId, limit: parseFloat(newLimit) });
    setNewCatId('');
    setNewLimit('');
