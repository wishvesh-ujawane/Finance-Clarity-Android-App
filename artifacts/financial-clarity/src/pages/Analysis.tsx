...
function monthLabel(m: string) {
  const [y, mo] = m.split('-');
  return new Date(parseInt(y, 10), parseInt(mo, 10) - 1).toLocaleDateString('en-IN', { month: 'short' });
}
...
export default function Analysis() {
  ...
  const [activePanel, setActivePanel] = useState<'spending' | 'ai' | 'daily' | 'future'>('spending');
  ...
  const monthlyInsight = useMemo(() => {
    ...
    const sourceText = topIncreases.length > 0
      ...
      : currentExpenses > 0
      ? 'No single category created a major spike this month.'
      : 'Add this month\'s expenses to unlock category drivers.';
    ...
    return {
      ...
      lines: [
        previousExpenses > 0
          ...
          : currentExpenses > 0
          ? 'This is your first month with spending data in this comparison window.'
  ...
  useEffect(() => {
    ...
    if (view === 'expense' || view === 'income') setViewType(view);
  }, [location]);
  ...
  return (
    <div className="p-4 md:p-6 pb-24 md:pb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {[
          ...
          >
            ...
            <p className={cn('text-lg font-bold', stat.color)} style={{ fontFamily: 'var(--font-display)' }}>
              {stat.value}
            </p>
          </motion.div>
        ))}
      ...
      <Tabs value={activePanel} onValueChange={value => setActivePanel(value as 'spending' | 'ai' | 'daily' | 'future')}>
        <TabsList className="mb-4 gap-2" data-testid="analysis-tabs">
          <TabsTrigger value="spending">Spending</TabsTrigger>
          <TabsTrigger value="ai">AI Card</TabsTrigger>
          <TabsTrigger value="daily">Daily Safe</TabsTrigger>
          <TabsTrigger value="future">Future Visibility</TabsTrigger>
        </TabsList>
              ...
              {chartData.every(d => d.value === 0) ? (
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={chartData} barSize={28} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                    ...
                    />
                    <Tooltip
                      formatter={(value: number) => [formatINR(value), viewType === 'expense' ? 'Spent' : 'Earned']}
                      contentStyle={{
                        borderRadius: '12px',
                        border: 'none',
                      ...
                      cursor={{ fill: 'hsl(214, 32%, 91%)', radius: 6 }}
                    />
        ...
        <TabsContent value="ai">
          <motion.div
            ...
            data-testid="monthly-ai-summary"
          >
        ...
        <TabsContent value="daily">
          <motion.div
            ...
            className="bg-card border border-border rounded-2xl p-5 mb-4"
            data-testid="daily-safe-spend"
          >
            <div className="flex items-center justify-between gap-3 mb-4">
              <div>
                <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">Daily Safe to Spend</p>
                <h2 className="text-sm font-bold text-foreground mt-1" style={{ fontFamily: 'var(--font-display)' }}>
                  Today
                </h2>
              </div>
        ...
        <TabsContent value="future">
          <motion.div
            ...
            className="bg-card border border-border rounded-2xl p-5 mb-4"
            data-testid="upcoming-obligations"
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Future Visibility</p>
                <h2 className="text-sm font-bold text-foreground" style={{ fontFamily: 'var(--font-display)' }}>
            ...
            {upcomingObligations.length === 0 ? (
              <div className="py-5 text-sm text-muted-foreground">
                No upcoming EMIs, credit card dues, subscriptions, bills, or SIPs found in future transactions.
              </div>
            ) : (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground">
                      {formatDateLabel(transaction.date, { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}
                    </p>
